...
def sound_for_GA(children, f_border, n, air_voice_path, ear_voice_path, throat_voice_path):
    ...
    for num in range(len(children)):
        ...
        sf.write("myvoice"+str(num+1)+".wav", data_myvoice, samplerate)
...
#自己聴取音のパラメーターの情報をtxtファイルに出力をする関数
def txt_information(children, children_num, f_border, folder):
    ...
    f.write("体内音→自己聴取音の重み："+format(child[6], '.2f')+"\n")
    f.write("気導音→自己聴取音の重み："+format(child[7], '.2f')+"\n\n")
