...
class SelectSounds(BoxLayout):
    ...
    def Actionchange_main(self):
        if(len(glob.glob('myvoice[1-8].wav'))==8):
            self.clear_widgets()
...
class End_body(BoxLayout):
    ...
    def to_myvoice_text(self):
        self.ids["label1"].text = "自己聴取音を生成中です。(20秒ほどかかります)"
...
class SoundPlayer_myvoice(BoxLayout):
    ...
    def __init__(self, **kwargs):
        ...
        self.text = "自己聴取音ができました。音を聞いてください。"
    ...
    #5段階評価をする関数
    def evaluation(self, id_num, eval_num):
        if(self.text == "自己聴取音ができました。音を聞いてください。" or self.text == "全ての音に対して評価してください。"):
            label_id = "myvoice"+str(id_num)+"_view"
            self.ids[label_id].text = str(eval_num)
    ...
    def ok(self):
        ...
        for id_num in range(1,9):
            ...
            txt = self.ids[label_id].text
            if txt != "-":
                evaluation_list.append(int(txt))
        if(len(evaluation_list) == 8 and (self.text == "自己聴取音ができました。音を聞いてください。" or self.text == "全ての音に対して評価してください。")):
            self.text = "自己聴取音を生成中です。(20秒ほどかかります)"
        return evaluation_list

    #OKボタンが押されたときに次の世代に進むか判定する関数
    def next_epoch(self, evaluation_list):
        if(self.text == "自己聴取音を生成中です。(20秒ほどかかります)"):
            global children
            gen_sound.rm_othersounds()
            children = GA.ga(children, evaluation_list, n, elite_num, roulette_num, BLX_num, random_num, alpha)
            gen_sound.sound_for_GA(children, f_border, n, air_voice_path, ear_voice_path, throat_voice_path)
            ...
            for id_num in range(1,9):
                label_id = "myvoice"+str(id_num)+"_view"
                self.ids[label_id].text = "-"
        elif len(evaluation_list) != 8 and self.text == "自己聴取音ができました。音を聞いてください。":
            self.text = "全ての音に対して評価してください。"
...
class End(BoxLayout):
    ...
    def app_end(self):
        txt = self.ids["aaa"].text
        if txt != "-":
            num = int(txt.replace("自己聴取音", ""))
            gen_sound.make_best_sound(num, folder)
            gen_sound.txt_information(children, num, f_border, folder)
            gen_sound.rm_allsounds()
            exit()
#kvファイルとの同期づけ
class MyApp(App):
